...
import { Float, PresentationControls, ContactShadows, QuadraticBezierLine, RoundedBox } from '@react-three/drei';
...
function AppleMac() {
  ...
  const screenColor = '#dcf2e3';
  const keyLight = '#dfd2c4';
  const keyDark = '#a89d91';
  const deskColor = '#deb887'; // wooden tone
  ...
  const screenMat = useMemo(() => new THREE.MeshStandardMaterial({ color: screenColor, emissive: screenColor, emissiveIntensity: 0.2, roughness: 0.2 }), []);
  ...
  return (
    <group ref={groupRef} position={[0, -0.5, 0]}>
      ...
      <group position={[0, 1.5, 0]}>
        ...
        <mesh position={[0, 0.7, 0.96]} material={ventMat}>
          <boxGeometry args={[1.75, 1.35, 0.05]} />
        </mesh>

        {/* The Screen Surface */}
        <mesh position={[0, 0.7, 0.98]} material={screenMat}>
          <boxGeometry args={[1.65, 1.25, 0.05]} />
        </mesh>
  ...
  return (
    <Canvas camera={{ position: [0, 4, 8], fov: 40 }} shadows>
      ...
      <PresentationControls
        ...
        snap={true}
        speed={1}
        zoom={0.9} // Zoom out a touch to encompass the desk and mouse
